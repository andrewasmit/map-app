...
import { PLACES_API_KEY, OPENTRIP_API_KEY, OPENTRIP_LIST_BASE_URL } from "@env";
...
function Autocomplete({ myLocation, setMyLocation, geocode }) {
  ...
  const fetchNearbyPlaces = () => {
    ...
    fetch(
      `${OPENTRIP_LIST_BASE_URL}lon=${lon}&lat=${lat}&format=json&limit=200&apikey=${OPENTRIP_API_KEY}`,
      {
        method: "GET",
        headers: {
  ...
  const fetchDetails = () => {
    const xid = "clikedOnObj.xid";

    fetch(`${OPENTRIP_DETAILS_BASE_URL}${xid}?apikey=${OPENTRIP_API_KEY}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
      },
